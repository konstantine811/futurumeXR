  ...
  return (
    <div className="flex flex-col md:flex-row h-full gap-8 animate-in fade-in">
          ...
          {parts.map((part) => (
            <button
              ...
              className={`absolute -translate-x-1/2 -translate-y-1/2 transition-transform hover:scale-110
                                ${
                                  part.type === "border"
                                    ? "w-full h-full rounded-[40%_60%_70%_30%] border-4 border-transparent hover:border-foreground/50"
            ...
            >
              {selectedPart === part.id && (
                <div className="absolute inset-0 ring-4 ring-foreground rounded-inherit animate-ping"></div>
              )}
            </button>
          ))}
        </div>
      ...
      <div className="w-full md:w-80 bg-popover p-6 rounded-2xl border border-border flex flex-col">
        <h3 className="text-muted-foreground uppercase font-bold text-xs mb-4">
          Аналіз об'єкта
        </h3>
        {selectedPart ? (
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 h-full animate-in fade-in overflow-y-auto">
      {organelles.map((item, i) => (
        <div
          key={i}
          className={`p-6 rounded-xl border border-border bg-popover/50 ${item.borderHover} transition-colors`}
        >
          <div
            className={`${item.textColor} font-bold uppercase text-xs tracking-widest mb-2`}
          >
            {item.role}
          </div>
      ...
      <div className="flex-1 flex flex-col md:flex-row gap-8 items-center justify-center">
        ...
        <div className="w-[300px] h-[300px] md:w-[400px] md:h-[400px] bg-green-900/10 border-4 border-dashed border-green-500/30 rounded-[40%_60%_70%_30%] relative flex items-center justify-center transition-all duration-500">
          ...
          {placed.includes("nucleus") && (
            <div className="absolute w-24 h-24 bg-purple-500 rounded-full flex items-center justify-center animate-in zoom-in font-bold text-xs text-foreground">
              Ядро
            </div>
          )}
  ...
  <div className="flex flex-col items-center justify-center h-full animate-in fade-in">
    ...
    <div className="space-y-6 max-w-2xl w-full">
      <div className="flex items-center gap-6 p-4 bg-popover rounded-xl border border-border relative">
        ...
        <div className="w-16 h-16 bg-purple-500 rounded-full flex items-center justify-center text-foreground font-bold shrink-0 z-10">
          1
        </div>
        <div>
      ...
      <div className="flex items-center gap-6 p-4 bg-popover rounded-xl border border-border relative">
        <div className="w-16 h-16 bg-red-500 rounded-full flex items-center justify-center text-foreground font-bold shrink-0 z-10">
          2
        </div>
        <div>
      ...
      <div className="flex items-center gap-6 p-4 bg-popover rounded-xl border border-border relative">
        <div className="w-16 h-16 bg-blue-500 rounded-full flex items-center justify-center text-foreground font-bold shrink-0 z-10">
          3
        </div>
        <div>
          <h3 className="text-xl font-bold text-blue-400">ЕПС (Транспорт)</h3>
          <p className="text-muted-foreground">
            Перевозить білок до цеху пакування.
          </p>
        </div>
      </div>
      <div className="flex items-center gap-6 p-4 bg-popover rounded-xl border border-border relative">
        <div className="w-16 h-16 bg-yellow-500 rounded-full flex items-center justify-center text-foreground font-bold shrink-0 z-10">
          4
        </div>
        <div>
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in items-center">
      ...
      <div className="w-full max-w-2xl bg-popover rounded-2xl p-6 min-h-[100px] border border-border flex flex-col gap-2">
        <span className="text-xs text-muted-foreground uppercase font-bold text-center">
        ...
        {userOrder.map((id, idx) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={`flex-1 p-3 rounded-lg ${item?.color} text-foreground font-medium`}
              >
                {item?.label}
              </div>
            </div>
  ...
  return (
    <div className="flex flex-col md:flex-row gap-8 h-full animate-in fade-in">
      <div className="flex-1 flex flex-col gap-6">
        ...
        <button
          onClick={heal}
          className="mt-auto py-3 border border-border rounded-lg hover:bg-muted text-muted-foreground"
        >
          Скинути симуляцію
        </button>
      </div>
      ...
      <div className="flex-1 bg-background rounded-2xl border border-border p-8 flex flex-col items-center justify-between relative overflow-hidden">
        ...
        >
          {energy < 30 && (
            <div className="absolute inset-0 bg-background/50 backdrop-grayscale transition-all"></div>
          )}
        </div>
      </div>
    </div>
    ...
    <div className="flex flex-col items-center justify-center h-full animate-in fade-in max-w-xl mx-auto">
      ...
      <div className="w-full bg-popover p-8 rounded-2xl border border-border">
        ...
        {answered && currentQ < questions.length - 1 && (
          <button
            onClick={next}
            className="mt-6 w-full py-3 bg-accent text-background font-bold rounded-lg hover:brightness-110"
          >
            Далі
          </button>
        )}
    ...
    <div className="flex flex-col items-center h-full animate-in fade-in max-w-2xl mx-auto text-center">
      ...
      <div className="w-full bg-popover p-6 rounded-2xl border border-border text-left">
        ...
        <input
          ...
          className="w-full bg-muted border border-border rounded-lg p-3 text-foreground mb-4 focus:border-accent outline-none"
        />
        ...
        <textarea
          ...
          className="w-full bg-muted border border-border rounded-lg p-3 text-foreground h-32 focus:border-accent outline-none"
        ></textarea>
    ...
    <div className="space-y-6 w-full">
      <div>
        ...
        <div className="flex gap-4">
          <button className="flex-1 p-3 bg-popover rounded hover:bg-muted text-sm text-foreground">
            Складність процесів
          </button>
          <button className="flex-1 p-3 bg-popover rounded hover:bg-muted text-sm text-foreground">
            Схожість на завод
  ...
  <div className="h-full flex flex-col animate-in fade-in">
    ...
    <div className="grid md:grid-cols-3 gap-6">
      ...
      <div className="bg-popover p-6 rounded-2xl border border-border hover:border-blue-500 transition-colors group relative overflow-hidden">
        ...
        <h3 className="text-xl text-foreground font-bold mb-4">
          Моделювальник
        </h3>
        <p className="text-muted-foreground text-sm mb-6">
